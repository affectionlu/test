...
page_init(void)
{
	...
	//PAGSIZE~npages_basemem*PGSIZE is free space
...
{  
    //check if there are free pages to allocate.
   if(page_free_list==NULL)
     return NULL;
   struct Page* page=page_free_list;
   page_free_list=page->pp_link;
   page->pp_link=0;
   //If (alloc_flags & ALLOC_ZERO),returned physical page with '\0' bytes.
...
page_free(struct Page *pp)
{
    ...
    //first check is the pointers are valid.
    if(pp->pp_link!=0||pp->pp_ref!=0)
      panic("page free is not right");
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
     ...
     pde_t pde=pgdir[PDX(va)];//get the content in the pdir array for a given va
     //check is pte table is present
     if(!(pde&PTE_P)){
      if(!create){
         return NULL;
      }
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
    ...
    int i;
    pte_t * pte;
    //check is is size is multiple of PGSIZE
    if(size%PGSIZE){
      panic("size must be mutiple of PGSIZE\n");
     }
    for(i=0;i<(size/PGSIZE);i++)
    {
      pte=pgdir_walk(pgdir,(void*)(va+i*PGSIZE),creat);//此时获得的pte虚拟地址
      if(!pte)
        panic("Out of memory\n");
      *pte=pa|PTE_P|perm;
      pa+=PGSIZE;
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm)
{
  ...
  //check if the page table alloctation succeeded.
  if(!pte)
    return -E_NO_MEM;
 //check if there is already a page mapped at 'va', it should be page_remove()d.
  if(*pte&PTE_P)
  {
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	struct Page *p;
	pte_t * pte_store;
	uint32_t udown=(uint32_t)ROUNDDOWN(va,PGSIZE);
	for(;udown<(uint32_t)ROUNDUP(va+len+1,PGSIZE);udown+=PGSIZE)
	{
      p=page_lookup(env->env_pgdir,(void *)udown,&pte_store);
	  if(udown>=ULIM||p==NULL||(*pte_store&perm)!=perm)
	  {
   	      user_mem_check_addr = udown > (uintptr_t) va ? udown: (uintptr_t) va;
		  return -E_FAULT;
	  }
	}
	return 0;
